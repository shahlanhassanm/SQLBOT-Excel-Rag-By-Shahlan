...
Test count: **272 → 634 passing** (+362) (root suite), `backend/tests` 28/28 throughout. Zero failures; deterministic across repeated runs.
...
| **D-11** | measurement | `backend/tests/bird_eval.py:73` | **Strict EX flips on float aggregates.** Same SQL, same data, 5 consecutive runs: **2 of 5 disagreed** (`459.9562642112432` vs `459.95626421124325`, column type `real`). ±1 question of irreducible noise. | ✅ | 📖 **WONTFIX** <br>→ **WONTFIX by your decision** — modifying `calculate_ex` would break compatibility with the official BIRD implementation, which you required. Mitigated rather than fixed: **EX-tolerant** is the stable internal KPI, `calculate_f1_tolerant` pairs with it (E-09), and `docs/BENCHMARK-BIRD.md` now states that differences below ~3 pp are noise and that McNemar on paired outcomes beats comparing headline percentages. |
...
| **D-40** *(new, 2026-08-01)* | measurement | `backend/tests/bird_eval.py` | **Pipeline runs recorded a model they did not use.** `--model` only feeds the `chat()` helper used by `--mode model`; in `--mode pipeline` the model comes from SQLBot's `ai_model` row where `default_model` is true. `run_provenance` wrote the `--model` value regardless, so a run launched `--model gpt-oss:20b` produced a file *claiming* gpt-oss while actually running `qwen2.5-coder:32b-20k`. A file asserting a model it did not use is worse than one asserting nothing — the exact failure E-06 exists to prevent. Caught during verification, before any mislabelled results were kept. | ✅ | **FIXED** <br>→ `pipeline_model_in_use()` reads the configured model and provenance now records `model` (actual), `model_requested`, `model_source` and `model_mismatch`. The harness also prints the model and a loud warning at launch, so the mismatch is visible before the GPU time is spent. |
| **D-41** *(new, 2026-08-01)* | tooling | `backend/tests/bird_run_supervised.sh` | **The supervisor stacked concurrent harnesses.** Killing it on the host kills the `docker exec` client but NOT the python process inside the container, which keeps running, holds the GPU and writes the same results file. Each relaunch added another: **five concurrent `bird_eval` processes** were observed on one 24 GB vGPU sharing one output file, which froze the run on question 1. The script exists to be restart-safe, so this defeated its only purpose. | ✅ | **FIXED** <br>→ the supervisor now kills any orphaned harness inside the container before starting one. |
...
| **C-07** *(new)* | test integrity | `backend/tests/test_row_rag_e2e.py` | **The audit's "zero flaky tests" claim is wrong.** This e2e test failed twice immediately after a bulk `docker cp` of the backend tree, then passed 5 consecutive times with no code change. It performs real embedding + pgvector retrieval, and the host runs `OLLAMA_KEEP_ALIVE=0`, so the embedding model unloads when idle and the first call after a pause pays a full reload — visible as the test's own runtime swinging 26 s → 52 s. Not a regression from any Phase 5 change: it passes in isolation, in every pairwise combination with its predecessors, and in the full suite once the model is warm. | 📖 | ⚠️ **DOCUMENTED** <br>→ Fix would be to pre-warm the embedding model in a fixture, or mark it `@pytest.mark.slow` and exclude it from the gate. Not done here: it changes the meaning of the suite, which is a call for the project owner. Treat a lone failure of this test as environmental until reproduced on a warm model. |
