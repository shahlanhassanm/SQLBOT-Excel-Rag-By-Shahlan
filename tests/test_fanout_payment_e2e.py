"""Full end-to-end fanout: ask "list all the payment gateways" in an auto chat and
...
def main():
    ...
    body = json.dumps({'chat_id': chat_id, 'question': 'list all the payment gateways'}).encode()
    ...
    assert len(rows) >= 40, f"expected ~55 unique gateways, got {len(rows)} rows"
