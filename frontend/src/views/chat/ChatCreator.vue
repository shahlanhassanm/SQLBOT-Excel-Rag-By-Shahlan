...
function confirmSelectDs() {
  // auto-route: no datasource to validate, create an unbound chat directly
  if (innerDs.value === AUTO_DS) {
    createChat(null)
    return
  }
  if (innerDs.value) {
...
// sentinel datasource id meaning "auto-route across all datasources"
const AUTO_DS = -1

function selectAuto() {
  // select auto-route like any other card; the Confirm button then proceeds
  innerDs.value = AUTO_DS
}

function selectAutoDirectly() {
  innerDs.value = AUTO_DS
  confirmSelectDs()
}

function createChat(datasource: number | null) {
  loading.value = true
  const param = {
  ...
  <div v-loading.body.fullscreen.lock="loading || statusLoading">
    ...
    >
      ...
      <div v-if="datasourceListWithSearch.length" class="card-content">
        <div
          v-if="!assistantStore.getAssistant"
          class="mb-16 auto-route-card"
          :style="{
            border:
              innerDs === AUTO_DS
                ? '2px solid var(--ed-color-primary, #1cba90)'
                : '1px solid var(--ed-color-primary, #1cba90)',
            borderRadius: '8px',
            padding: '14px 16px',
            cursor: 'pointer',
            background:
              innerDs === AUTO_DS ? 'rgba(28, 186, 144, 0.16)' : 'rgba(28, 186, 144, 0.06)',
          }"
          @click="selectAuto"
          @dblclick="selectAutoDirectly"
        >
          <div style="font-weight: 600; font-size: 14px; display: flex; align-items: center">
            <span>⚡ {{ $t('qa.auto_datasource') }}</span>
            <el-icon
              v-if="innerDs === AUTO_DS"
              style="margin-left: 8px; color: var(--ed-color-primary, #1cba90)"
            >
              <icon_done_outlined />
            </el-icon>
          </div>
          <div style="font-size: 12px; color: #646a73; margin-top: 4px">
            {{ $t('qa.auto_datasource_tip') }}
          </div>
        </div>
