...
const headerRowOptions = computed(() => {
  const raw = previewData.value?.rawHead ?? []
  return raw.map((row: string[], i: number) => {
    const preview = row
    ...
    return {
      value: i,
      label: `${t('ds.row')} ${i}${preview ? ` — ${preview}` : ''}`,
    }
  })
})

const onHeaderRowChange = (newRow: number) => {
  if (!previewData.value || newRow == null) return
  if (newRow === previewData.value.headerRow) return
  loading.value = true
  datasourceApi
    .reparseExcel({
      filePath,
      sheetName: previewData.value.sheetName,
      headerRow: newRow,
    })
    .then((res: any) => {
      const updated = res.data ?? res
      const idx = arr.findIndex((s: any) => s.sheetName === updated.sheetName)
      if (idx >= 0) {
        arr[idx] = updated
        previewData.value = updated
        fieldList.value = updated.fields
      }
    })
    .finally(() => {
      loading.value = false
  ...
  >
    ...
    <div v-loading="loading" class="content">
      ...
      <div class="preview-num">
        ...
        <span
          v-if="headerRowOptions.length > 1"
          class="header-row-picker"
        >
          <span class="header-row-picker__label">{{ t('ds.header_row') }}</span>
          <el-select
            :model-value="previewData.headerRow"
            size="small"
            style="width: 280px"
            @change="onHeaderRowChange"
          >
            <el-option
              v-for="opt in headerRowOptions"
              :key="opt.value"
              :label="opt.label"
              :value="opt.value"
            />
          ...
          <span
            v-if="previewData.headerConfidence != null && previewData.headerConfidence < 0.62"
            class="header-row-picker__warn"
          >
            {{ t('ds.header_low_confidence') }}
          </span>
        </span>
      </div>
