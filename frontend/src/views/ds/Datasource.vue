...
// --- bulk Excel upload: each file becomes its own datasource ---
const { wsCache } = useCache()
const bulkUploadURL = import.meta.env.VITE_API_BASE_URL + '/datasource/addExcelDatasource'
const bulkHeaders = ref<any>({ 'X-SQLBOT-TOKEN': `Bearer ${wsCache.get('user.token')}` })
const bulkPending = ref(0)
const bulkOk = ref(0)
const bulkFail = ref(0)

const beforeBulkUpload = (file: any) => {
  const ok = /\.(xlsx|xls|csv)$/i.test(file.name) && file.size / 1024 / 1024 <= 50
  if (!ok) {
    ElMessage.error(`${file.name}: ${t('common.not_exceed_50mb')} (.xlsx/.xls/.csv)`)
    return false
  }
  bulkPending.value++
  return true
...
const finishBulkOne = () => {
  bulkPending.value--
  if (bulkPending.value <= 0) {
    if (bulkOk.value > 0) {
      ElMessage.success(t('datasource.bulk_upload_done', { ok: bulkOk.value, fail: bulkFail.value }))
    } else if (bulkFail.value > 0) {
      ElMessage.error(t('datasource.bulk_upload_done', { ok: bulkOk.value, fail: bulkFail.value }))
    }
    bulkOk.value = 0
    bulkFail.value = 0
    search()
...
const onBulkError = (e: any) => {
  bulkFail.value++
  console.error('bulk excel upload failed:', e)
  finishBulkOne()
}
    ...
    <div class="datasource-methods">
      ...
      <div class="button-input">
        ...
        <el-upload
          ...
          :headers="bulkHeaders"
          :before-upload="beforeBulkUpload"
          :on-success="onBulkSuccess"
          :on-error="onBulkError"
          :show-file-list="false"
          accept=".xlsx,.xls,.csv"
          multiple
          style="margin-right: 12px"
        >
          <el-button secondary :loading="bulkPending > 0">
            <template #icon>
              <icon_upload_outlined></icon_upload_outlined>
            </template>
            {{ $t('datasource.bulk_excel_upload') }}
          </el-button>
        </el-upload>
