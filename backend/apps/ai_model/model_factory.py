...
def invoke_small_llm(prompt: str, model_name: str = "") -> str:
    """Like invoke_default_llm but optionally overrides the MODEL NAME while reusing
    the default endpoint + credentials — lets a lightweight task (e.g. header-row
    detection) run on a small local model without a separate ai_model row. Empty
    model_name falls back to the default model."""
    import asyncio
    loop = asyncio.new_event_loop()
    try:
        config = loop.run_until_complete(get_default_config())
        ...
        loop.close()
    if model_name:
        # LLMConfig is a frozen pydantic model — copy with the override, don't mutate.
