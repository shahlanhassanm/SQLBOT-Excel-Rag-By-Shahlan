...
async def start_chat(session: SessionDep, current_user: CurrentUser, create_chat_obj: CreateChat):
    """Start a chat. ``datasource`` may be omitted: the chat then runs in
    auto-route mode — the agentic finder selects the best datasource for every
    question (and can decompose across several). Workspace ownership of an
    explicit datasource is enforced inside create_chat."""
    try:
        return create_chat(session, current_user, create_chat_obj,
                           require_datasource=bool(create_chat_obj.datasource))
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=str(e)
