...
def create_chat(session: SessionDep, current_user: CurrentUser, create_chat_obj: CreateChat,
    ...
    # Create the first (empty) record for bound-datasource chats AND for
    # auto-route chats (no datasource). The frontend only shows the message
    # input once the chat has at least one record, so auto chats need this
    # placeholder too — they just carry no datasource / recommended data.
    if (require_datasource and ds) or not require_datasource:
        # generate first empty record
        record = ChatRecord()
        record.chat_id = chat.id
        record.datasource = ds.id if ds else None
        record.engine_type = ds.type_name if ds else ''
        record.first_chat = True
        record.finish = True
        record.create_time = datetime.datetime.now()
        record.create_by = current_user.id
        if ds and isinstance(ds, CoreDatasource) and ds.recommended_config == 2:
