...
class CreateChat(BaseModel):
    # Optional so the frontend may send an explicit null (auto-route chats post
    # {"datasource": null}); a bare `int = None` makes Pydantic v2 reject null
    # with 422 because the default only applies when the field is omitted.
    id: Optional[int] = None
    question: Optional[str] = None
    datasource: Optional[int] = None
...
class AiModelQuestion(BaseModel):
    ...
    def planning_sys_question(self):
        return get_sql_template()['planning_system'].format(lang=self.lang, sqlbot_name=self.sqlbot_name)

    def planning_user_question(self):
        return get_sql_template()['planning_user'].format(lang=self.lang, question=self.question)

    def planning_aggregate_sys_question(self):
        return get_sql_template()['planning_aggregate_system'].format(lang=self.lang)

    def planning_aggregate_user_question(self, draft_plans: str):
        return get_sql_template()['planning_aggregate_user'].format(
            lang=self.lang, question=self.question, draft_plans=draft_plans)

    def prune_delete_sys_question(self):
        return get_sql_template()['prune_delete_system']

    def prune_delete_user_question(self, schema_batch: str):
        return get_sql_template()['prune_delete_user'].format(
            question=self.question, logical_plan=self.logical_plan or '(no plan available)',
            schema_batch=schema_batch)

    def prune_select_sys_question(self):
        return get_sql_template()['prune_select_system']

    def prune_select_user_question(self, schema_batch: str):
        return get_sql_template()['prune_select_user'].format(
            question=self.question, logical_plan=self.logical_plan or '(no plan available)',
            schema_batch=schema_batch)

    def profile_role_sys_question(self):
        return get_sql_template()['profile_role_system']

    def profile_role_user_question(self, table_schema: str):
        return get_sql_template()['profile_role_user'].format(
            question=self.question, logical_plan=self.logical_plan or '(no plan available)',
            table_schema=table_schema)

    def profile_probe_sys_question(self, roles: str):
        return get_sql_template()['profile_probe_system'].format(
            engine=self.engine, roles=roles)

    def profile_probe_user_question(self, table_schema: str, quoted_table_ref: str,
                                    quoted_columns_block: str):
        return get_sql_template()['profile_probe_user'].format(
            question=self.question, logical_plan=self.logical_plan or '(no plan available)',
            table_schema=table_schema, quoted_table_ref=quoted_table_ref,
            quoted_columns_block=quoted_columns_block)

    # --- agentic pipeline prompts (grader / retry / decompose / synthesize) ---
    def grader_sys_question(self):
        return get_sql_template()['grader_system'].format()

    def grader_user_question(self, sql: str, result_preview: str):
        return get_sql_template()['grader_user'].format(
            question=self.question, sql=sql, result_preview=result_preview)

    def sql_retry_user_question(self, feedback: str, current_time: str):
        return get_sql_template()['retry_user'].format(
            feedback=feedback, current_time=current_time)

    def alt_candidate_hint(self):
        return get_sql_template()['alt_candidate_hint']

    def decompose_sys_question(self, primary_ds_id: int):
        return get_sql_template()['decompose_system'].format(primary_ds_id=primary_ds_id)

    def decompose_user_question(self, candidates_json: str):
        return get_sql_template()['decompose_user'].format(
            question=self.question, candidates_json=candidates_json)

    def synthesize_sys_question(self):
        return get_sql_template()['synthesize_system'].format(lang=self.lang)

    def synthesize_user_question(self, original_question: str, legs_json: str, mode: str = 'split'):
        return get_sql_template()['synthesize_user'].format(
            original_question=original_question, legs_json=legs_json, mode=mode)
    ...
    def sql_user_question(self, current_time: str, change_title: bool):
        ...
        if self.logical_plan:
            _plan_block = get_sql_template().get('logical_plan_block', '').format(logical_plan=self.logical_plan)
        _obs_block = ''
        if self.data_observations:
            _obs_block = get_sql_template().get('data_observations_block', '').format(
                observations=self.data_observations)
        return get_sql_template()['user'].format(lang=self.lang, engine=self.engine, schema=self.db_schema,
                                                 ...
                                                 logical_plan_block=_plan_block,
                                                 data_observations_block=_obs_block)
