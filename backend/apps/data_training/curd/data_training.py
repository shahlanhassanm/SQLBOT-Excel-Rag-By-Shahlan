...
from apps.data_training.skeleton import rerank_by_skeleton
...
# Candidate pool for few-shot selection. The LIMIT is a bind parameter so the
# skeleton re-ranker can widen the pool without changing how many examples
# finally reach the prompt.
embedding_sql = f"""
...
def _fewshot_pool_size() -> int:
    """How many candidates to pull before re-ranking. Without the re-ranker the
    pool IS the answer, so it stays at the configured example count."""
    top_count = settings.EMBEDDING_DATA_TRAINING_TOP_COUNT
    if not settings.SKELETON_FEWSHOT_ENABLED:
        return top_count
    return max(top_count, settings.SKELETON_FEWSHOT_POOL)
...
def select_training_by_question(session: SessionDep, question: str, oid: int, datasource: Optional[int] = None,
            ...
            try:
                ...
                if advanced_application_id is not None:
                    ...
                    results = session.execute(text(embedding_sql),
                                              {'embedding_array': str(embedding), 'oid': oid,
                                               'datasource': datasource, 'top_n': _top_n})
    ...
    # Iterate _ids, not _map: _ids preserves relevance order (ILIKE hits first,
    # then embedding hits by descending similarity) whereas _map follows the
    # arbitrary row order of the IN(...) query above. Feeding the re-ranker — or
    # the prompt — in DB order would discard the ranking entirely.
    _results: list[dict] = [_map[_id] for _id in _ids if _id in _map]

    if settings.SKELETON_FEWSHOT_ENABLED:
        # Domain terms are not passed: this runs before the schema is chosen, so
        # only literals/numbers/dates are masked. Still the bulk of the win, and
        # schema-aware masking can be added if selection ever moves after
        # choose_table_schema().
        _results = rerank_by_skeleton(question, _results,
                                      top_k=settings.EMBEDDING_DATA_TRAINING_TOP_COUNT)
    else:
        _results = _results[:settings.EMBEDDING_DATA_TRAINING_TOP_COUNT]
